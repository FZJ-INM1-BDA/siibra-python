...
siibra.find_maps(space="MNI Colin 27")
...
julich_mpm = siibra.get_map(
    space="icbm 2009c", parcellation="julich 3.1", maptype="labelled"
)
print(julich_mpm)
...
plotting.plot_roi(
    mapimg_provider.get_data(), title=f"{julich_mpm.parcellation.name}", cmap=cmap
)
...
mapimg_r = julich_mpm.extract_regional_map("hoc1 right")
plotting.plot_roi(mapimg_r.get_data(), title=f"hoc1 (right)")
...
with siibra.QUIET:  # suppress progress output
    julich_pmaps = siibra.get_map(
        space="icbm 152", parcellation="3.1", maptype="statistical", name="227"
    )

pmap_imgprovider = julich_pmaps.extract_regional_map(julich_pmaps.regionnames[0])
plotting.plot_stat_map(pmap_imgprovider.get_data(), cmap="viridis")
...
v1left_imgprovider = julich_pmaps.extract_regional_map("v1 left")
...
plotting.plot_roi(v1left_imgprovider.get_data(), title="v1 left", cmap="viridis")
