...
http_requests_args = [
    ("http://foo.co/bar", None, None, False, False, {})
]
    ...
    try:
        with patch("builtins.open", m_open):
            with patch("os.rename", rename_mock):
                with patch.object(HttpRequest, "cached", new_callable=PropertyMock, return_value=cache_flag) as pmock:
                    with patch.object(CACHE, "build_filename", return_value=return_filename) as build_filename_mock:
...
def test_httprequests_retrieve(cache_flag, post_flag):
    ...
    m_open = mock_open()
    rename_mock = MagicMock()
    with patch_all(post_flag=post_flag, cache_flag=cache_flag) as (req, m_open, rename_mock, pmock, build_filename_mock, response_text):
        req._retrieve()
...
def test_file_lock_(filenames,presleeps,expected_indices):
    ...
    call_order = []
    def exec_retrieve(req: HttpRequest, overwrite_cachefile=None, presleep=None, index=None):
    ...
    with patch_all() as (req, m_open, rename_mock, pmock, build_filename_mock, response_text):
        
        rename_mock.side_effect = rename_mock_side_effect

        with ThreadPoolExecutor(max_workers=2) as ex:
            _calls_returns = list(ex.map(
                exec_retrieve,
                repeat(req),
                filenames,
                presleeps,
