...
def test_get_hoc1_left_density():
    ...
    assert len(features) == 1, "expect only 1 result from getting hoc1 left receptor, but got {len(features)}"
    expected_substrings = ["receptor density", "Area hOc1"]
    assert all(s in features[0].name for s in expected_substrings),\
...
def test_no_receptor_data():
    ...
    assert len(human_receptor_fingerprints) >= 42, f"expect at least 42 receptor query data, but found only {len(human_receptor_fingerprints)}"
