...
def test_parcs_has_volumes(parc: Parcellation):
    ...
    assert all(
        vol.space is not None
        for vol in parc.volumes
    )
...
def test_parcs_get_region(idx_1,idx_2):
    assert julich_29_parcs[idx_1].regiontree.children[0] is not julich_29_parcs[idx_2].regiontree.children[0]
    
    # changing name attr does not affect other parcellations
    idx_1_name = julich_29_parcs[idx_1].regiontree.children[0].name
    assert julich_29_parcs[idx_1].regiontree.children[0].name == julich_29_parcs[idx_2].regiontree.children[0].name
    julich_29_parcs[idx_1].regiontree.children[0].name = 'foo-bar'
    assert julich_29_parcs[idx_1].regiontree.children[0].name != julich_29_parcs[idx_2].regiontree.children[0].name
    julich_29_parcs[idx_1].regiontree.children[0].name = idx_1_name
    assert julich_29_parcs[idx_1].regiontree.children[0].name == julich_29_parcs[idx_2].regiontree.children[0].name
