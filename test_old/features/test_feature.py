...
parc_inst = parcellation.Parcellation(
    "parc_id", "name", Species.UNSPECIFIED_SPECIES, regions=(reg_inst,)
)
...
list_of_queries = [
    ...
    (
        ...
        space_inst,
        None,
        f"lq0::FooFeature::s:{space_inst.id}::{feature_inst.id}",
    ),
    (
        feature_inst,
        parc_inst,
        None,
        f"lq0::FooFeature::p:{parc_inst.id}::{feature_inst.id}",
    ),
    (
        feature_inst,
        reg_inst,
        None,
        f"lq0::FooFeature::p:{parc_inst.id}::r:{reg_inst.name}::{feature_inst.id}",
    ),
]
...
@pytest.fixture
def mock_all(
    mock_parse_featuretype, mock_space_registry, mock_parcellation, mock_region
):
...
list_of_fids = [
    (
        f"bla::{feature_name}::{space_spec}::{feature_inst.id}",
        ParseLiveQueryIdException,
        None,
        None,
        None,
    ...
    (
        f"{lq_prefix}::{feature_name}::foo:bar::{feature_inst.id}",
        ParseLiveQueryIdException,
        None,
        None,
        None,
    ...
    (
        f"{lq_prefix}::{feature_name}::{reg_spec}::{feature_inst.id}",
        ParseLiveQueryIdException,
        None,
        None,
        None,
    ...
    (
        f"{lq_prefix}::{feature_name}::{space_spec}::{parc_spec}::{feature_inst.id}",
        ParseLiveQueryIdException,
        None,
        None,
        None,
    ...
    (
        f"{lq_prefix}::{feature_name}::{parc_spec}::{space_spec}::{feature_inst.id}",
        ParseLiveQueryIdException,
        None,
        None,
        None,
    ...
    (
        f"{lq_prefix}::{feature_name}::{reg_spec}::{parc_spec}::{feature_inst.id}",
        ParseLiveQueryIdException,
        None,
        None,
        None,
    ...
    (
        f"{lq_prefix}::{feature_name}::{parc_spec}::{feature_inst.id}",
        None,
        ["parc"],
        [parc_inst.id],
        parc_inst,
    ...
    (
        f"{lq_prefix}::{feature_name}::{space_spec}::{feature_inst.id}",
        None,
        ["space"],
        [space_inst.id],
        space_inst,
    ...
    (
        f"{lq_prefix}::{feature_name}::{parc_spec}::{reg_spec}::{feature_inst.id}",
        None,
        ["parc", "region"],
        [parc_inst.id, reg_inst.name],
        reg_inst,
        feature_inst.id,
    ),
]
...
@pytest.mark.parametrize(
    "fid,ExCls,mocks_called,args_used,return_concept,decoded_id", list_of_fids
)
def test_deserialize_query_context(
    fid, ExCls, mocks_called, args_used, return_concept, decoded_id, mock_all
):
    (
        mock_parse_featuretype,
        mock_space_registry,
        mock_parcellation,
        mock_region,
    ) = mock_all
    ...
    for key, mock in [
        ("space", mock_space_registry.__getitem__),
        ("parc", mock_parcellation.__getitem__),
        ("region", mock_region),
    ]:
