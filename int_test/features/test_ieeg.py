...
from siibra.core import Atlas, Region, atlas
...
def test_all_ieeg(ieeg: IEEG_Session):
    ...
    IEEG_Session.SiibraSerializationSchema(**basic_result)
    assert 'detail' not in basic_result

    detail_result = JSONEncoder.encode(ieeg, detail=True, nested=True)
    IEEG_Session.SiibraSerializationSchema(**detail_result)
    ...
    IEEG_Session.SiibraSerializationSchema(**roi_result)
