...
class Cache:
    ...
    def __init__(self):
        raise RuntimeError("Call instance() to access " f"{self.__class__.__name__}")
    ...
    def instance(cls):
        ...
        if cls._instance is None:
            ...
            if SKIP_CACHEINIT_MAINTENANCE:
                logger.debug(
                    "Will not run maintenance on cache as SKIP_CACHE_MAINTENANCE is set to True."
                )
            else:
    ...
    def run_maintenance(self):
        ...
        sfiles = sorted(
            [(fn, os.stat(fn)) for fn in files], key=lambda t: t[1].st_atime
        )
        ...
        if index > 0:
            logger.debug(
                f"Removing the {index + 1} oldest files to keep cache size below {targetsize:.2f} GiB."
            )
            for fn, st in sfiles[: index + 1]:
                if os.path.isdir(fn):
                    ...
                    size = sum(
                        os.path.getsize(f) for f in os.listdir(fn) if os.path.isfile(f)
                    )
                    shutil.rmtree(fn)
                else:
                    size = st.st_size
