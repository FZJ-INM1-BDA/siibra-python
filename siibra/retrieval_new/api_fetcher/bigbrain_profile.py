...
from ...locations.layerboundary import LayerBoundary, X_PRECALCULATED_BOUNDARY_KEY, LAYERS
...
def query_bigbrain_profile(input: AttributeCollection):
    ...
    ptcld = PointCloud(space_id="minds/core/referencespace/v1.0.0/a1655b99-82f1-420f-a3c2-fe80fd4c8588",
                       coordinates=vertices.tolist())
    root_coords = np.array(ptcld.coordinates)
    dtype = {
        "names": ["x", "y", "z"],
        "formats": [root_coords.dtype] * 3
    }
    ...
    for attr in input.attributes:
        ...
        if isinstance(matched, Pt):
            matched = PointCloud(space_id=matched.space_id,
                                 coordinates=[matched.coordinate])
        if isinstance(matched, PointCloud):
            ...
            std = [matched_profiles[layer_labels == layer].std() for layer in range(1, 7)]

            dataframe = pd.DataFrame(np.array([means, std]).T,
                                     columns=["mean","std"],
                                     index=LAYERS[1: -1])
            attr = Tabular(extra={
                X_DATA: dataframe,
                X_BIGBRAIN_LAYERWISE_INTENSITY: True
            })
            attributes.append(attr)
            
            for index in coordidx.tolist():
                _profile = profile[index]
                depth = np.arange(0., 1., 1. / (profile[index].shape[0]) )
                
                tabular_attr = Tabular(extra={
                    X_DATA: pd.DataFrame(_profile, index=depth),
                    X_BIGBRAIN_PROFILE_VERTEX_IDX: index
                })
                layer_boundary = LayerBoundary(extra={
...
def get_thickness(url: str):
    assert url.endswith(".npy"), f"Thickness URL must end with .npy"
    ...
    boundary_depths = np.c_[np.zeros_like(valid), (thickness[valid, :] / total_thickness[valid, None]).cumsum(1)]
...
def get_profile(url: str, valid: np.ndarray):
    assert url.endswith(".npy"), f"Profile URL must end with .npy"
    resp = requests.get(url)
    resp.raise_for_status()
...
def get_vertices(url: str, valid: np.ndarray):
    assert url.endswith(".gii"), f"Vertices URL must end with .gii"
    resp = requests.get(url)
    resp.raise_for_status()
