...
"""A 3D plane sitting in 3D space with zero thickness."""
...
from . import base, point, pointcloud, patch
...
def from_points(p1: point.Point, p2:point.Point, p3:point.Point):
    ...
    """
    n = np.cross(
        (p2.warp(p1.space) - p1).coordinate,
        (p3.warp(p1.space) - p1).coordinate,
    )
    n = n / np.linalg.norm(n)
    d = np.dot(p1.coordinate, n)
...
def from_image(image: "image.ImageProvider"):
    ...
    """
    im_lowres = image.extract_imagedata(resolution=1)
    ...
    return from_points(points[0], points[1], points[2])
