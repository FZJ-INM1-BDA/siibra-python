...
class Plane3D:
    ...
    def intersect_mesh(self, mesh: dict):
        ...
        face_indices = np.where(face_vertex_in_halfspace.min(1) != face_vertex_in_halfspace.max(1))[0]
        ...
        crossings = np.diff(face_vertex_in_halfspace[face_indices][:, [2, 0, 1, 2]], axis=1)
        ...
        fwd_intersections = self.intersect_line_segments(
            mesh["verts"][fwd_vertices[:, 0]], 
            mesh["verts"][fwd_vertices[:, 1]]
        )
        bwd_intersections = self.intersect_line_segments(
            mesh["verts"][bwd_vertices[:, 0]], 
            mesh["verts"][bwd_vertices[:, 1]]
        )
        ...
        while len(face_indices) > 0:
            ...
            labels.append(
                (face_id, fwd_vertices[face_id, 0], fwd_vertices[face_id, 1])
            )
    ...
    def from_image(cls, image: volume.Volume):
        ...
        voxels = pointset.PointSet(
            np.vstack(([0, 0, 0], np.identity(3)[plane_dims])),
            space=None
        )
