...
class MapAssignment:
    ...
    def __init__(
        ...
        result: CompareMapsResult = None
    ):
        self.input_structur = input_structure
        self.centroid = centroid
        self.volume = volume
        self.fragment = fragment
        self.map_value = map_value
        self.result = result or CompareMapsResult()
...
class Map(concept.AtlasConcept, configuration_folder="maps"):
    ...
    def _assign_points(self, points: pointset.PointSet, lower_threshold: float) -> List[MapAssignment]:
        ...
        for pointindex, pt in siibra_tqdm(
            ...
            if sigma_vox < 3:
                ...
                for entry in self._run_assignment(W, lower_threshold=lower_threshold):
    ...
    def _assign_map(
        ...
        """
        Assign another (single-volume & labelled) parcellation map to this parcellation map.
        ...
        """
        # assumes a single-volume labelled parcellation as source
        assert len(srcmap.volumes) == 1
        assert srcmap.space == self.space
        assert srcmap.maptype == MapType.LABELLED
        assignments = []
        ...
        for srcregion in siibra_tqdm(
            ...
            candidates = self._assign_image(
                mask, minsize_voxel=1, lower_threshold=0, split_components=False, quiet=True
            )
            for c in candidates:
    ...
    def from_nifti(
        ...
        name: str
            Human-readable name of the parcellation
        nifti_map: str of Nifti1Image
            NIfTI file with a labelled volume.
        space_spec: str
            Specification of the reference space (name, keyword, or id - e.g. 'mni152')
        regionnames: list of string
            List of human-readable names of the mapped regions
        regionlabels: list of int
            List of integer labels in the nifti file corresponding to the list of regions
        """
        def get_uuid(s):
            if isinstance(s, str):
                b = s.encode("UTF-8")
            elif isinstance(s, Nifti1Image):
                b = s.to_bytes()
            else:
                raise ValueError(f"Cannot build uuid for parameter type {type(s)}")
            hex_string = hashlib.md5(b).hexdigest()
            return str(uuid.UUID(hex=hex_string))

        spaceobj = space.Space.registry().get(space_spec)
        if not isinstance(nifti_map, Nifti1Image):
            assert isinstance(nifti_map, str) and path.isfile(nifti_map)
            nifti_map = Nifti1Image.load(nifti_map)
        arr = np.asanyarray(nifti_map.dataobj)
        ...
        for label, regionname in zip(regionlabels, regionnames):
            if label not in mapped_labels:
                logger.warn(f'Label {label} not mapped in the provided NIfTI volume -> region {regionname} is removed from the map.')
            elif label in indices.values():
                logger.warn(f'Label {label} already defined, will not map it to {regionname}.')
            else:
        ...
        logger.warn(
            'The following labels appear in the NIfTI volume, but not in the specified regions: '
            f'{", ".join(str(l) for l in unnamed_labels)}. They will be removed from the map.'
        )
        ...
        # TODO consider the case that the parcellation itself already exists
        ...
        parcmap = Map(
            ...
            space_spec=spaceobj.spec,
            parcellation_spec=parcobj.spec,
            indices={n: [{'label': l}] for n, l in zip(regionnames, regionlabels)},
            volumes=[_volume.Volume(spaceobj.spec, providers=[provider])]
        )
    ...
    def compute_relations(self, other_parcellation: Union[str, "Parcellation"]):
        ...
        """
        if isinstance(other_parcellation, str):
