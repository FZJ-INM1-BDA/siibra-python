...
class ZipfileRequest(HttpRequest):
    def __init__(self, url, filename: str = "", func=None):
        ...
        HttpRequest.__init__(
            self, url, func=func or self.find_suitiable_decoder(self.filename)
        )
        self.cachefile = CACHE.build_filename(
            self.url + self.filename.replace("/", "_")
        )
        self._zipfile = None

    @property
    def zipfile(self) -> RemoteZip:
        """Stream the remote zipfile"""
        if self._zipfile is None:
            session = requests.Session()
            self._zipfile = RemoteZip(self.url, session=session)
