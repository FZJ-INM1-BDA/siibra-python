...
from ...attributes.datarecipes.volume import ImageRecipe
...
class BigBrainProfile(LiveQuery[Feature], generates=Feature):
    ...
    def generate(self) -> Iterator[Feature]:
        ...
        image_recipes = [
            recipe
            for recipes in self.find_attributes(ImageRecipe)
            for recipe in recipes
        ]

        if len(image_recipes) == 0:
            return

        if len(image_recipes) > 1:
            logger.info(f"Found {len(image_recipes)}, using the first one only.")

        image_recipe = image_recipes[0]

        warped_vertices = bigbrain_vertices.warp(image_recipe.space_id)
        indices, values = image_recipe.lookup_points(warped_vertices)
        filtered_indices = np.array(indices)[np.nonzero(values)]

        if len(filtered_indices) == 0:
            return

        filtere_ptcld = PointCloud(
            coordinates=vertices[filtered_indices],
            space_id=BIGBRAIN_ID,
        )

        attributes = [*modalities_of_interest, filtere_ptcld]

        matched_profiles = profiles[filtered_indices]
        matched_boundary_depths = boundary_depths[filtered_indices]
        ...
        hashed_io = md5(
            (json.dumps(image_recipe.ops) + json.dumps(asdict(filtere_ptcld))).encode(
                "utf-8"
            )
        ).hexdigest()
