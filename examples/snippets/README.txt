...
.. topic:: Necessary preparations
	...
	While several of the functionalities do not require EBRAINS access, we will assume in the examples that you have set the environment variable `HBP_AUTH_TOKEN` to a freshly obtained EBRAINS access token, or directly pass it via `siibra.set_ebrains_token(<your token here>)`. The token is a string sequence with more than 1000 characters, which usually begins with something like "eyJhbGciOiJSUzI1NiIsInR...". 
