...
# %%
# Fiber bundles are anchored by assigning their terminal points to brain areas
# in supported reference atlases. This allows siibra to query bundles by brain
# area. Here, we query bundles terminating in area 3b (PostCG) in the left
# hemisphere.
...
# %%
# The modality attribute can be used to filter only the short bundles. We use
# this here to select short superficial bundle terminating in area 3b left. The
# bundle is represented as a pandas DataFrame, containing x/y/z coordinates of
# streamlines in MNI space. Streamlines are concatenated and indexed by their
# fiber id. Coordinates of each streamline are ordered.
...
# %%
# The coordinates of streamlines can be plotted using nilearn. We color them by
# fiber id to distinguish different streamlines. Overlay terminal regions of the
# bundle are listed in the anchor attribute of the data feature. We plot these
# as contours for reference.
...
def add_region_contours(regions, display, linewidth=1):
    ...
    for c, r in zip(colors, regions):
        display.add_contours(
            r.get_regional_map("mni152").fetch(), colors=[c], linewidths=linewidth
        )
    patches = [
...
add_region_contours(bundle.anchor.regions, display, linewidth=1.0)


# %%
# Long bundles can be selected in the same fashion. We plot the left CST bundle
# and its terminal regions in separate panels.
...
display = plotting.plot_markers(
    bundle.data.index, bundle.data.values, axes=ax1, **bundle_plot_settings
)
display.title(bundle.name.replace("_", " "), size=12)
