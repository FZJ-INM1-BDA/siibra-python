...
fts = siibra.features.get(regions[0], siibra.features.connectivity.FunctionalConnectivity)
conn = fts[0]
...
D1.iloc[:17]['mean'].plot(**kwargs, yerr=D1.iloc[:17]['std'], ax=a1, ylabel=shorten_name(regions[0].name))
D2.iloc[:17]['mean'].plot(**kwargs, yerr=D2.iloc[:17]['std'], ax=a2, ylabel=shorten_name(regions[1].name))
