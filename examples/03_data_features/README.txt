...
``siibra`` provides access to data features of different modalities using ``siibra.features.get()``, which accepts an **anatomical concept** like a brain region, parcellation, or reference space, and a feature modality as listed in the ``siibra.modalities`` registry. 
...
``siibra`` implements a unified handling for querying different types of features, broadly categorized into
